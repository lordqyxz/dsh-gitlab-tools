...
- **better-sidebar 标签**（`betterSidebar.registerTab`，id `gitlab-tools:issues`）：与 资源管理器 / Git / 子代理 / 终端 / 浏览器 等标签平级，通过 `ctx.inject(['betterSidebar'])` 等服务就绪后注册（better-sidebar 0.18+ 客户端半区可能晚于旧轮询窗口挂载，固定延迟重试会静默丢 tab）。打开即显示 `defaultProject` 的打开中 issue，**卡片式列表**（标题 / #iid / 指派人 / 标签 / 更新时间），点卡片展开标签内详情+讨论，可手动刷新 + 按设置间隔自动轮询（标签未激活时暂停轮询）。
...
- **声明式「功能设置」（better-sidebar 侧边卡片）**：标签描述符带 `settings.pluginToggles`——「默认项目」（文本，`group/project`）与「刷新间隔」（数字，5000–3600000ms）直接在 better-sidebar 设置页该卡片底部的「功能设置」条里编辑，持久化在 better-sidebar 自有 `pluginSettings["gitlab-tools:issues"]`，改动即时生效（标签组件经 `store.subscribe` 订阅）。**优先级**：侧边栏功能设置 > 设置页（host 路由）同名配置 > 内置默认（120000ms）。
- **设置页**（`settings.section` 槽位，设置 →「GitLab Issues」）：`defaultProject` / `refreshMs` 作为回退配置仍在（侧边栏功能设置未填时生效）；host/token/项目→本地文件夹映射也在这里，带「测试连接」。
