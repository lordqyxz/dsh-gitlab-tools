...
export function buildMentionPrompt({ project, kind, issue, note }) {
  ...
  return [
    ...
    '- 本地代码检出：当前会话工作目录即该仓库（启动时同步，可能落后远端）。需要代码级核对先 `git fetch origin`；分析特定 MR 再 `git fetch origin <分支>`，不要假设本地已是最新。',
    '',
    '### 描述',
    '',
    '> ' + desc.split('\n').join('\n> '),
...
export function buildPipelinePrompt({ project, pipeline, mergeRequest, failedJobs }) {
  ...
  return [
    ...
    '- 本地检出在会话工作目录（main 分支，启动时同步可能落后）：先 `git fetch origin`，再 `git fetch origin ' + mergeRequest.source_branch + '` 即可本地核对该 MR 的改动。',
...
export function createResponder({
  ...
  async function markFailed(event, kind, sessionId, err) {
    ...
    try {
      await ack.fallbackComment(target, failureBody(msg))
    } catch (e) {
  ...
  async function injectSession(prompt, event, kind, iid) {
    ...
    if (ack) {
      try {
        ...
        rememberAck(state, sessionId, { project: event.project, kind, iid, noteId, awardId, emoji: ACK_EYES })
      } catch (e) {
    ...
    saveState(stateFile, state)
    return { sessionId, workspaceNote }
  }
  ...
  async function handle(event) {
      ...
      if (!agent) {
        ...
        if (ack) {
          try {
            ...
            rememberAck(state, existing, { project: event.project, kind, iid: promptArgs.issue.iid, noteId, awardId, emoji: ACK_EYES })
          } catch (e) {
            logger?.warn?.('[gitlab-tools] ack award failed: ' + String((e && e.message) || e))
          }
        }
    ...
    let workspaceNote
    try {
      ({ sessionId, workspaceNote } = await injectSession(prompt, event, kind, promptArgs.issue.iid))
    } catch (e) {
      await markFailed(event, kind, null, e)
      return 'inject-failed:' + String((e && e.message) || e).slice(0, 100)
    }
    recordRate(key, 'responded')
    return `responded:${sessionId}${workspaceNote}`
  }
 ...
 */
export function createListener({ client, agents, controller, store, state, stateFile, projects, intervalMs, mentionUsername, saUsername, resolveCwd, ensureWorkspace, logger, ack, pipelineTriage, schedule, maxPerIssuePerHour, maxContinuationsPerHour }) {
  const responder = createResponder({ agents, controller, store, state, stateFile, mentionUsername, saUsername, resolveCwd, ensureWorkspace, logger, ack, client, pipelineTriage, maxPerIssuePerHour, maxContinuationsPerHour })
  ...
  async function injectProbe() {
    const out = { agentsService: false, controllerService: false, workspaceService: false, agent: false, prompt: false, workspace: null, sessionId: null, error: null }
    try {
