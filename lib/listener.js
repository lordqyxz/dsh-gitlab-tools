...
export function createResponder({
  ...
  async function handlePipeline(event) {
    ...
    const { sessionId, workspaceNote } = await injectSession(prompt, event, 'mr', mr.iid)
  ...
  async function handle(event) {
    ...
    const { sessionId, workspaceNote } = await injectSession(prompt, event, kind, promptArgs.issue.iid)
    recordRate(key, 'responded')
 ...
 */
export function createListener({ client, agents, controller, store, state, stateFile, projects, intervalMs, mentionUsername, saUsername, resolveCwd, ensureWorkspace, logger, pipelineTriage, schedule, maxPerIssuePerHour, maxContinuationsPerHour }) {
  const responder = createResponder({ agents, controller, store, state, stateFile, mentionUsername, saUsername, resolveCwd, ensureWorkspace, logger, client, pipelineTriage, maxPerIssuePerHour, maxContinuationsPerHour })
