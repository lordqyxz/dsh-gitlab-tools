...
export function buildMentionPrompt({ project, kind, issue, note }) {
  ...
  return [
    ...
    'GitLab 查询/管理类操作可用工具（project=' + project + '）。本地会话工作目录即该仓库检出（启动时同步，可能落后远端；需要代码级核对先 git fetch origin，分析特定 MR 再 fetch 其分支）。大工件/大 JSON 先 bash 落盘再 jq 提取，不要整包读进对话。',
    '</system-reminder>',
  ].filter(Boolean).join('\n')
}
 ...
 */
export function createListener({ client, agents, controller, store, state, stateFile, projects, intervalMs, mentionUsername, saUsername, resolveCwd, ensureWorkspace, logger, ack, reply, pipelineTriage, schedule, maxPerIssuePerHour, maxContinuationsPerHour }) {
  const responder = createResponder({ agents, controller, store, state, stateFile, mentionUsername, saUsername, resolveCwd, ensureWorkspace, logger, ack, reply, client, pipelineTriage, maxPerIssuePerHour, maxContinuationsPerHour })
