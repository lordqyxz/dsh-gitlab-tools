 ...
 */
export function createOutbound({ client, state, stateFile, logger, ack, maxReplyLen = 20000, now = Date.now }) {
    ...
    try {
      ...
      if (!COMMITTABLE_REASONS.has(reason)) {
        ...
        if (info && ack) {
          ...
          if (!soft) {
            try {
              await ack.fallbackComment(info, failureBody(event.data?.reason))
            } catch (e) {
      ...
      if (!text) {
        ...
        if (info && ack) {
          ...
          try {
            await ack.fallbackComment(info, '⚠️ 处理完成但未生成回复，可重新 @ 我重试。')
          } catch (e) {
            logger?.warn?.('[gitlab-tools] ack fallback comment failed: ' + String((e && e.message) || e))
          }
          saveState(stateFile, state)
      ...
      const noteId = r?.data?.id ?? null
      const failed = !r?.data || r?.error
      state.outbound[sessionId] = {
        ...
        bytes: text.length,
        error: failed ? String(r?.error ?? 'empty-response').slice(0, 160) : null,
      }
      ...
      try {
        ...
        if (info && ack) {
          await ack.swap(info, ACK_FAILED)
          await ack.fallbackComment(info, failureBody(msg))
          saveState(stateFile, state)
        }
      } catch (e2) {
