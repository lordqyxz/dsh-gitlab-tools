 ...
 */
export function createOutbound({ client, state, stateFile, logger, maxReplyLen = 20000, now = Date.now }) {
  ...
  async function handleSessionEvent(session, event) {
    try {
      ...
      if (!COMMITTABLE_REASONS.has(reason)) return `skip-reason:${reason ?? 'unknown'}`
