...
import { foldUsage, formatUsageFooter } from './usage.js'
 ...
 */
export function createOutbound({ client, state, stateFile, logger, ack, reply, maxReplyLen = 20000, usageFooter = true, now = Date.now }) {
  ...
  async function handleSessionEvent(session, event) {
    try {
      ...
      // 用量脚注（agentUsageFooter，默认开）：从会话事件流折叠本轮 + 会话累计，固定格式
      // 附在回复末尾（追加在截断之后，不会被 maxReplyLen 截掉）。事件日志不可用（stash
      // 兜底路径）不加——数字必须可回溯，宁缺勿造，不伪装精确值。
      if (usageFooter && Array.isArray(session.events)) {
        const footer = formatUsageFooter(foldUsage(session.events, { turn }), foldUsage(session.events))
        if (footer) text = `${text}\n\n---\n\n${footer}`
      }
