    ...
    handler: async (req, res) => {
      ...
      if (pathname === "/gitlab-tools/session/prompt" && req.method === "POST") {
        ...
        try {
          ...
          const agentsSvc = ctx.agents ?? ctx.get?.("agents");
          if (agentsSvc?.create && !agentsSvc.get?.(sessionId)) {
            try {
              await agentsSvc.create({ sessionId });
              await run();
              return sendJson(res, 200, { ok: true, sessionId, queued: true, reattached: true });
            } catch (error2) {
              return sendJson(res, 200, { ok: false, code: "reattach-failed", message: String(error2?.message ?? error2) });
            }
          }
          return sendJson(res, 200, { ok: false, code: "prompt-failed", message: String(error?.message ?? error) });
        }
      }
