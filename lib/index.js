...
export async function apply(ctx, config) {
  ...
  const UiConfig = z.object({
    ...
    host: z.string().default(""),
    // 访问令牌（write-only）：设置页可设置/覆盖/清除，但 GET /settings 不回显
    // 明文（只回 tokenConfigured），避免把密钥下发到浏览器。
    token: z.string().default(""),
    // Panel auto-refresh interval (ms).
    refreshMs: z.number().min(5000).max(3600000).default(120000)
  });
  const uiBase = { defaultProject: cfg.defaultProject ?? "", host: cfg.host ?? "", token: cfg.token ?? "", refreshMs: 120000 };
  ...
  // 客户端按「有效 host/token = 设置里的（优先）|| 配置里的」构建，并按
  // host|token 记忆化：改 host 或 token 都会重建客户端。
  let clientCache = new Map();
  const getClient = () => {
    const u = uiConfig();
    const host = (u.host || cfg.host).trim();
    const token = (u.token || cfg.token).trim();
    const key = `${host}|${token}`;
    if (!clientCache.has(key)) {
      clientCache.set(key, createClient({ host, token }));
    }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (pathname === "/gitlab-tools/status" && isGet) {
        const u = uiConfig();
        const configured = Boolean((u.host || cfg.host).trim() && (u.token || cfg.token).trim());
        return sendJson(res, 200, { ok: true, configured });
      }

      if (pathname === "/gitlab-tools/settings" && isGet) {
        const u = uiConfig();
        // token 只写不回显：不回传明文，只回 tokenConfigured（供 UI 显示占位）。
        return sendJson(res, 200, {
          ok: true,
          defaultProject: u.defaultProject,
          host: u.host,
          refreshMs: u.refreshMs,
          tokenConfigured: Boolean(u.token || cfg.token)
        });
      }
      ...
      if (pathname === "/gitlab-tools/settings" && req.method === "POST") {
        ...
        const patch = {};
        for (const key of ["defaultProject", "host", "token", "refreshMs"]) {
          if (key in body) patch[key] = body[key];
        }
        if (Object.keys(patch).length === 0) {
          return sendJson(res, 400, { ok: false, code: "noop", message: "没有可写入的配置项" });
        }
        ...
        if ("token" in patch && typeof patch.token !== "string") {
        ...
        try {
          ...
          return sendJson(res, 200, { ok: true, ...uiConfig() });
        } catch (error) {
