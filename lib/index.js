...
export async function apply(ctx, config) {
  ...
  reg(defineTool({
    ...
    async execute(args = {}) {
      ...
      const head = [
        ...
        resp && resp.enabled ? `响应=on  mention=@${resp.mentionUsername}  限流=${resp.maxPerIssuePerHour}/h` : "响应=off（config.webhookMentionUsername 为空）",
        `总事件=${total}`,
  ...
  const listener = createListener({
    ...
    resolveCwd: (project) => {
      ...
      return (m?.dir && m.dir.trim()) || "/Users/apple/dev";
    },
  });
  ...
  if (cfg.webhookNtfyUrl) {
    ...
    listenerRef.current = { ...listener, ntfy };
    ctx.effect(() => {
      ntfy.start();
      return () => ntfy.stop();
