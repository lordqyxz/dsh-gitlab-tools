...
import { aiTokenRefKey, resolveAiIdentity } from "./identity.js";

export const name = "gitlab-tools";
export const inject = ["tools", "webServer", "settings", "credentials", "agents", "sessionController"];
...
export const Config = z.object({
  ...
  webhookSecretToken: z.string().default(""),
  // 事件落盘路径（JSONL；超 webhookMaxFileLines 行自动保留后半）。
  webhookEventsFile: z.string().default("~/.dsh/gitlab-tools/webhook-events.jsonl"),
  webhookMaxFileLines: z.number().min(100).default(2000),
  // 空 = 接受全部项目；非空 = 只接受列出的 path_with_namespace。
  webhookProjectWhitelist: z.array(z.string()).default([]),
  // ── 事件轮询源 + @mention 自动响应（可选）───────────────────────────────
  // 要监听的项目（path_with_namespace），空 = 轮询关闭。
  webhookPollProjects: z.array(z.string()).default([]),
  // 轮询间隔（ms，最小 10s）。游标与已见 note id 落盘，重启不重放。
  webhookPollIntervalMs: z.number().min(10000).default(30000),
  // @mention 触发的用户名（GitLab 服务账号用户名）；空 = 只记录事件、不自动响应。
  webhookMentionUsername: z.string().default(""),
  // ntfy 订阅源（推送）：主题 /json 流地址 + dsh token；空 = 关闭（轮询兜底仍在）。
  webhookNtfyUrl: z.string().default(""),
  webhookNtfyToken: z.string().default(""),
  // 响应会话默认工作目录（项目→文件夹映射未命中时用；空 = 回落 /Users/apple/dev）。
  webhookDefaultCwd: z.string().default(""),
  // MR 流水线失败自动分诊（推送源专属能力；轮询兜底不含 pipeline 事件）。
  webhookPipelineTriage: z.boolean().default(true),
  webhookPollStateFile: z.string().default("~/.dsh/gitlab-tools/webhook-poll-state.json")
});
...
export async function apply(ctx, config) {
  const cfg = {
    ...
    webhookEventsFile: config.webhookEventsFile ?? "~/.dsh/gitlab-tools/webhook-events.jsonl",
    webhookMaxFileLines: config.webhookMaxFileLines ?? 2000,
    webhookProjectWhitelist: Array.isArray(config.webhookProjectWhitelist) ? config.webhookProjectWhitelist : [],
    webhookPollProjects: Array.isArray(config.webhookPollProjects) ? config.webhookPollProjects : [],
    webhookPollIntervalMs: config.webhookPollIntervalMs ?? 30000,
    webhookMentionUsername: config.webhookMentionUsername ?? "",
    webhookNtfyUrl: config.webhookNtfyUrl ?? "",
    webhookNtfyToken: config.webhookNtfyToken ?? "",
    webhookDefaultCwd: config.webhookDefaultCwd ?? "",
    webhookPipelineTriage: config.webhookPipelineTriage ?? true,
    webhookPollStateFile: config.webhookPollStateFile ?? "~/.dsh/gitlab-tools/webhook-poll-state.json"
  };
  ...
  const webhookEventsFile = expandHome(cfg.webhookEventsFile);
  const eventStore = new EventStore({ eventsFile: webhookEventsFile, maxFileLines: cfg.webhookMaxFileLines });

  // ── GitLab webhook 接收器（可选功能）────────────────────────────────────
  // enabled=false 时路由分支回 404；事件存储始终存在（轮询源也写它）。
  const webhook = (() => {
    const secretToken = String(cfg.webhookSecretToken || "").trim();
    if (!secretToken) return { enabled: false, store: eventStore, eventsFile: webhookEventsFile };
    return {
      enabled: true,
      store: eventStore,
      eventsFile: webhookEventsFile,
      handle: createGitlabWebhookHandler({
        store: eventStore,
        secretToken,
        projectWhitelist: cfg.webhookProjectWhitelist,
  ...
  reg(defineTool({
    ...
    description: "查看最近收到的 GitLab webhook 事件（POST /gitlab-tools/webhook 接收端点落盘；插件 config 的 webhookSecretToken 非空时启用）。首行是接收器状态（文件路径/总数/类型分布），默认每条一行摘要；detail=true 附完整 payload JSON（每条截 4000 字符）。",
    parameters: {
      limit: { type: "number", description: "返回最近多少条，默认 20" },
      kind: { type: "string", description: "按 object_kind 过滤：push/note/issue/merge_request/pipeline/tag_push" },
      project: { type: "string", description: "按项目路径子串过滤（如 group/proj）" },
      detail: { type: "boolean", description: "true 时附完整 payload JSON" }
    },
    timeoutMs: cfg.timeoutMs,
    output: TEXT_OUTPUT,
    async execute(args = {}) {
      const L = listenerRef.current;
      const ntfy = L && L.ntfy ? L.ntfy.status() : null;
      const poll = L ? L.poller.status() : null;
      const resp = L ? L.responder.status() : null;
      const ob = L?.outbound?.status ? L.outbound.status() : null;
      const aiId = await getAiIdentity();
      const { total, matched } = webhook.store.query(args);
      const st = webhook.store.stats();
      const head = [
        `接收器状态：webhook=${webhook.enabled ? "on" : "off"}  文件=${webhook.eventsFile}`,
        poll && poll.enabled ? `轮询=on  项目=${poll.projects.join(", ")}  间隔=${poll.intervalMs}ms  lastPollAt=${poll.lastPollAt ?? "尚未轮询"}` : "轮询=off（config.webhookPollProjects 为空）",
        ntfy && ntfy.enabled ? `ntfy推送=on  ${ntfy.url}  已消费=${ntfy.messages}${ntfy.lastError ? "  最近错误=" + ntfy.lastError : ""}` : "ntfy推送=off（config.webhookNtfyUrl 为空）",
        resp && resp.enabled ? `响应=on  mention=@${resp.mentionUsername}  限流=${resp.maxPerIssuePerHour}/h${resp.pipelineTriage ? "  pipeline分诊=on" : "  pipeline分诊=off"}` : "响应=off（config.webhookMentionUsername 为空）",
        ob ? `出站桥=on  绑定会话=${ob.boundSessions}  已贴回=${ob.postedCount}${ob.lastTarget ? "  最近=" + ob.lastTarget : ""}${ob.lastError ? "  最近错误=" + ob.lastError : ""}` : "出站桥=off（无 session/event 订阅）",
        `AI身份=@${aiId.username || "?"}（${aiId.source}）${aiId.warning ? "  ⚠ " + aiId.warning : ""}`,
        `总事件=${total}`,
        st.lastReceivedAt ? `最近一条=${st.lastReceivedAt}` : "（还没收到过事件）",
        st.total ? `类型分布=${JSON.stringify(st.byKind)}` : "",
        cfg.webhookProjectWhitelist.length ? `白名单=${cfg.webhookProjectWhitelist.join(", ")}` : "白名单=(全部接受)"
      ].filter(Boolean).join("  ");
      if (!matched.length) return res(head + "\n\n(无匹配事件)", { enabled: true, total, events: [] });
      const body = matched.map((r) => {
        const line = summarizeRecord(r);
        if (!args.detail) return line;
        let payloadText = "";
        try { payloadText = JSON.stringify(r.payload); } catch { payloadText = "(payload 不可序列化)"; }
        return `${line}\n  payload: ${payloadText.slice(0, 4000)}`;
      });
      return res([head, "", ...body].join("\n"), { enabled: true, total, events: matched });
    }
  }));

  reg(defineTool({
    name: "gitlab_webhook_poll_now",
    ...
    async execute() {
      ...
      if (!L || !L.poller.status().enabled) {
        return res("轮询未启用：在插件 config（cordis.patch.yml 的 gitlab-tools insert）配置 webhookPollProjects 后生效。", { enabled: false });
      }
  ...
  const UiConfig = z.object({
    ...
    // 本客户端选中的 AI 身份（服务账户 username；空 = 跟随 AI token 本尊）
    aiUsername: z.string().default(""),
    // 本客户端保存过 token 的账户（下拉记忆项；由保存流程维护，不直接可写）
    aiAccounts: z.array(z.string()).default([])
  });
  const uiBase = { defaultProject: cfg.defaultProject ?? "", host: cfg.host ?? "", refreshMs: 120000, projectDirs: [], aiUsername: "", aiAccounts: [] };
  ...
  const uiConfig = () => {
    ...
    return {
      ...
      aiUsername: s.aiUsername ?? "",
      aiAccounts: Array.isArray(s.aiAccounts) ? s.aiAccounts : []
    };
  };
  ...
  // AI 专属 token 旧链（DeepSeek Harness 身份）：legacy REF → cfg.aiToken。
  const getLegacyAiToken = async () => {
    if (ctx.credentials) {
      try {
  ...
  // token → 用户名（GET /user），按 token 记忆化（有界）。
  const tokenUsernameCache = new Map();
  const resolveTokenUsername = async (token) => {
    const key = String(token || "");
    if (!key) return "";
    if (tokenUsernameCache.has(key)) return tokenUsernameCache.get(key);
    let username = "";
    try {
      const client = await makeClient(key);
      const r = await client.raw({ path: "/api/v4/user", method: "GET" });
      username = (r.data && r.data.username) || "";
    } catch {
      username = "";
    }
    if (tokenUsernameCache.size > 20) tokenUsernameCache.clear();
    tokenUsernameCache.set(key, username);
    return username;
  };

  // 生效 AI 身份：设置页所选账户（aiUsername）→ token 按账户分存。
  const getAiIdentity = async () => {
    const u = uiConfig();
    let perAccountToken = "";
    if (u.aiUsername && ctx.credentials) {
      try {
        const r = await ctx.credentials.resolve(credentialRef(aiTokenRefKey(u.aiUsername)));
        perAccountToken = (r && r.value) || "";
      } catch {
        perAccountToken = "";
      }
    }
    const legacyToken = await getLegacyAiToken();
    const legacyUsername = await resolveTokenUsername(legacyToken);
    const cfgTokenU = cfg.aiToken || "";
    const cfgUsername = cfgTokenU ? await resolveTokenUsername(cfgTokenU) : "";
    return resolveAiIdentity({
      aiUsername: u.aiUsername,
      perAccountToken,
      legacyToken,
      legacyUsername,
      cfgToken: cfgTokenU,
  ...
  const getAiToken = async () => (await getAiIdentity()).token;
  ...
  const listenerStateFile = expandHome(cfg.webhookPollStateFile);
  ...
  const listener = createListener({
    ...
    projects: cfg.webhookPollProjects,
    intervalMs: cfg.webhookPollIntervalMs,
    mentionUsername: cfg.webhookMentionUsername,
    pipelineTriage: cfg.webhookPipelineTriage,
    ...
    resolveCwd: (project) => {
      ...
      return (m?.dir && m.dir.trim()) || cfg.webhookDefaultCwd.trim() || "/Users/apple/dev";
    },
  });
  ...
  if (cfg.webhookNtfyUrl) {
    const ntfy = createNtfySource({
      url: cfg.webhookNtfyUrl,
      token: cfg.webhookNtfyToken,
      store: eventStore,
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      // GitLab webhook 接收端点（可选功能：config.webhookSecretToken 非空才启用）。
      // handler 完全自管响应（writeHead+end），不走 sendJson。
      if (pathname === "/gitlab-tools/webhook") {
        if (!webhook.enabled) {
          return sendJson(res, 404, { ok: false, code: "webhook-disabled", message: "webhook 功能未启用：在插件 config 配置 webhookSecretToken" });
        }
      ...
      if (pathname === "/gitlab-tools/settings" && isGet) {
        const u = uiConfig();
        const token = await getToken();
        const aiIdentity = await getAiIdentity();
        ...
        return sendJson(res, 200, {
          ...
          aiUsername: u.aiUsername || "",
          aiAccounts: Array.isArray(u.aiAccounts) ? u.aiAccounts : [],
          aiIdentity: aiIdentity.username || "",
          aiIdentitySource: aiIdentity.source,
          aiIdentityWarning: aiIdentity.warning || "",
          tokenConfigured: Boolean(token.trim()),
          aiTokenConfigured: Boolean(aiIdentity.token.trim())
        });
      }
      ...
      if (pathname === "/gitlab-tools/settings" && req.method === "POST") {
        ...
        if ("aiToken" in body) {
          ...
          try {
            if (body.aiToken === "") {
              ...
              // 清除当前所选账户的 per-account token，并解除选择（回到跟随模式）
              const cur = (uiConfig().aiUsername || "").trim();
              if (cur) await ctx.credentials.unset(credentialRef(aiTokenRefKey(cur)));
              if (!("aiUsername" in body)) body.aiUsername = "";
            } else {
              const tokenUser = await resolveTokenUsername(body.aiToken);
              if (!tokenUser) {
                // 离线/网络失败无法验证身份：仍存 legacy REF（旧行为），标注未验证
                await ctx.credentials.set(credentialRef(AITOKEN_REF), body.aiToken);
                aiIdentityNotice = "AI token 已保存，但其身份未能验证（GitLab 查询失败）；网络恢复后重新保存一次可完成账户绑定";
              } else {
                const wanted = "aiUsername" in body ? String(body.aiUsername || "").trim() : "";
                if (wanted && wanted.toLowerCase() !== tokenUser.toLowerCase()) {
                  return sendJson(res, 400, { ok: false, code: "ai-username-mismatch", message: "token 属于 @" + tokenUser + "，与所选 @" + wanted + " 不一致；请填入所选账户自己的 PAT" });
                }
                await ctx.credentials.set(credentialRef(aiTokenRefKey(tokenUser)), body.aiToken);
                if (!("aiUsername" in body)) body.aiUsername = tokenUser;
              }
            }
          } catch (error) {
        ...
        // AI 身份选择（下拉）：仅切换以哪个账户发言；token 必须已存在。
        if ("aiUsername" in body) {
          if (typeof body.aiUsername !== "string") {
            return sendJson(res, 400, { ok: false, code: "config", message: "aiUsername 必须是字符串" });
          }
          const wanted = body.aiUsername.trim();
          if (wanted === "") {
            await scope.update({ aiUsername: "" });
          } else {
            let has = false;
            if (ctx.credentials) {
              try {
                const d = await ctx.credentials.describe(credentialRef(aiTokenRefKey(wanted)));
                has = Boolean(d && d.configured);
              } catch {
                has = false;
              }
            }
            if (!has) {
              const legacy = await getLegacyAiToken();
              const legacyUser = await resolveTokenUsername(legacy);
              if (legacyUser && legacyUser.toLowerCase() === wanted.toLowerCase()) has = true;
            }
            if (!has) {
              return sendJson(res, 400, { ok: false, code: "no-token-for-account", message: "@" + wanted + " 还没有 token：请先在「AI 专属 token」里填入该账户的 PAT（会自动绑定到所选账户）" });
            }
            await scope.update({ aiUsername: wanted });
            const accs = (uiConfig().aiAccounts || []).filter((x) => x !== wanted);
            accs.unshift(wanted);
            await scope.update({ aiAccounts: accs.slice(0, 20) });
          }
        }
        ...
        if (Object.keys(patch).length === 0) {
          ...
          return sendJson(res, 200, { ok: true, ...uiConfig(), tokenConfigured: Boolean((await getAiToken()).trim()), aiTokenConfigured: Boolean((await getAiToken()).trim()), ...(aiIdentityNotice ? { aiIdentityNotice } : {}) });
        }
        try {
      ...
      // ── AI 身份下拉候选：GitLab 服务账户（admin 全量）+ 本机已存 token 的账户 ──
      if (pathname === "/gitlab-tools/service-accounts" && isGet) {
        if (!cfg.host || !(await getToken()).trim()) {
          return sendJson(res, 200, { ok: false, code: "not_configured", message: "未配置 GitLab host/token（见 profile patch config）" });
        }
        const groups = [];
        let listingSource = "local";
        try {
          const c = await getUIClient();
          const r = await c.raw({ path: "/api/v4/service_accounts", query: { per_page: 100 } });
          if (Array.isArray(r.data) && r.data.length) {
            groups.push(r.data.map((a) => ({ username: a.username, name: a.name || "", id: a.id ?? null, source: "admin" })));
            listingSource = "admin";
          }
        } catch {
          // 非 admin token / 接口不可用：回落到本机已知账户
        }
        groups.push((uiConfig().aiAccounts || []).map((username) => ({ username, name: "", id: null, source: "stored" })));
        const aiId0 = await getAiIdentity();
        if (aiId0.username) {
          groups.push([{ username: aiId0.username, name: "", id: null, source: "current-ai", hasToken: Boolean(aiId0.token) }]);
        }
        try {
          const c = await getUIClient();
          const r = await c.raw({ path: "/api/v4/user", method: "GET" });
          if (r.data && r.data.username) {
            groups.push([{ username: r.data.username, name: r.data.name || "", id: r.data.id ?? null, source: "self" }]);
          }
        } catch {
          // 主 token 身份解析失败：跳过
        }
        const flat = groups.reduce((m, g) => { m.push(...g); return m; }, []);
        for (const a of flat) {
          if (!a.hasToken) {
            let has = false;
            if (ctx.credentials) {
              try {
                const d = await ctx.credentials.describe(credentialRef(aiTokenRefKey(a.username)));
                has = Boolean(d && d.configured);
              } catch {
                has = false;
              }
        ...
        for (const a of flat) {
          ...
          if (!key || seenAcct.has(key)) continue;
          seenAcct.add(key);
          accounts.push(a);
        }
        return sendJson(res, 200, { ok: true, source: listingSource, accounts });
      }
