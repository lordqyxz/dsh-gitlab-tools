...
export async function apply(ctx, config) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      // ── stats2：诊断探针——全 try/catch + stack 透传（排查 400 用，稳定后可删）──
      if (pathname === "/gitlab-tools/session/stats2" && isGet) {
        const sessionId = url.searchParams.get("sessionId") ?? "";
        try {
          const sessionsSvc = ctx.sessions ?? ctx.get?.("sessions");
          ...
          return sendJson(res, 200, { ok: true, sessionId, loaded: Boolean(sess), eventCount: events.length, running: Boolean(agent), agentType: typeof agent });
        } catch (e) {
          return sendJson(res, 200, { ok: false, code: "stats2", message: String(e?.message ?? e), stack: String(e?.stack ?? "").slice(0, 900) });
        }
      }
      ...
      if (pathname === "/gitlab-tools/session/prompt" && req.method === "POST") {
        ...
        const run = () => controller.prompt({
          ...
          // 网关 normally 注入 abort signal；直连调用必须自带，否则控制器内部
          // signal.throwIfAborted() 对 undefined 调用直接崩（实测服务器）。
