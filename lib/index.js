...
export async function apply(ctx, config) {
  ...
  reg(defineTool({
    name: "gitlab_agent_events",
    description: "查看最近收到的 GitLab webhook 事件（POST /gitlab-tools/webhook 接收端点落盘；插件 config 的 agentSecretToken 非空时启用）。首行是接收器状态（文件路径/总数/类型分布），默认每条一行摘要；detail=true 附完整 payload JSON（每条截 4000 字符）。",
    parameters: {
    ...
    async execute(args = {}) {
      ...
      const head = [
        ...
        poll && poll.enabled ? `轮询=on  项目=${poll.projects.join(", ")}  间隔=${poll.intervalMs}ms  lastPollAt=${poll.lastPollAt ?? "尚未轮询"}` : "轮询=off（config.agentPollProjects 为空）",
        ntfy && ntfy.enabled ? `ntfy推送=on  ${ntfy.url}  已消费=${ntfy.messages}${ntfy.lastError ? "  最近错误=" + ntfy.lastError : ""}` : "ntfy推送=off（config.agentNtfyUrl 为空）",
        resp && resp.enabled ? `响应=on  mention=@${resp.mentionUsername}  限流=${resp.maxPerIssuePerHour}/h${resp.pipelineTriage ? "  pipeline分诊=on" : "  pipeline分诊=off"}` : "响应=off（config.agentMentionUsername 为空）",
        ...
        cfg.agentProjectWhitelist.length ? `白名单=${cfg.agentProjectWhitelist.join(", ")}` : "白名单=(全部接受)"
      ].filter(Boolean).join("  ");
