...
export async function apply(ctx, config) {
  ...
  // Lazy, memoized client build (auth comes straight from config; no glab spawn).
  let clientPromise;
  const getClient = () => (clientPromise ??= createClient(cfg));
  ...
  const uiBase = { defaultProject: cfg.defaultProject ?? "", refreshMs: 120000 };
  ...
  const uiConfig = () => {
    ...
    const s = scope.get();
    return { defaultProject: s.defaultProject ?? "", refreshMs: s.refreshMs ?? 120000 };
  };
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (pathname === "/gitlab-tools/status" && isGet) {
        return sendJson(res, 200, { ok: true, configured: Boolean(cfg.host && cfg.token) });
      }
      ...
      if (pathname === "/gitlab-tools/settings" && req.method === "POST") {
        ...
        const patch = {};
        for (const key of ["defaultProject", "refreshMs"]) {
