...
export async function apply(ctx, config) {
  ...
  // 客户端按「有效 host = 设置里的 host（优先）|| 配置 host」构建，并按 host
  // 记忆化：改 host 会重建客户端，token 仍来自 profile patch 配置。
  let clientCache = new Map();
  const getClient = () => {
    const host = (uiConfig().host || cfg.host).trim();
    if (!clientCache.has(host)) {
      clientCache.set(host, createClient({ host, token: cfg.token }));
    }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (pathname === "/gitlab-tools/status" && isGet) {
        const effectiveHost = uiConfig().host || cfg.host;
        return sendJson(res, 200, { ok: true, configured: Boolean(effectiveHost && cfg.token) });
      }
