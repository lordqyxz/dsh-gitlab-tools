...
9. **HTTP 路由**（宿主 `lib/index.js` 里注册，`kind:'prefix' path:'/gitlab-tools'`）：
   ...
   - `POST /gitlab-tools/session/prompt?sessionId=&text=` → 向既有会话注入 queue 消息（重启后恢复被中断会话/续跑用，与 session/stop 对称；先直接 prompt——GUI 发消息同一 API，agent 未挂载时兜底 agents.create 重挂 loop；text ≤4000 字符，无鉴权因为路由只绑 127.0.0.1）
...
- **部署恢复（2026-09-17）**：docker restart 会停掉运行中的 agent loop（会话数据在盘上不丢，GUI 可重开）。恢复 = 对重启前活跃的会话逐个 `POST /gitlab-tools/session/prompt?sessionId=<id>&text=<续跑指令>`（源码 lib/index.js，与 stop 对称；loop 未挂载时路由内 agents.create 兜底重挂，响应带 reattached 位）；生效验证 = `GET /gitlab-tools/session/stats?sessionId=` 的 running 位。重启后 GUI boot token 会换，从 docker logs 新 boot 段取。
